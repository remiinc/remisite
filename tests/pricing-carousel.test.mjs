...
test('pricing media uses a four-image Embla carousel with autoplay progress and dots', async () => {
  ...
  assert.match(source, /const placeholderMessages = \['This is a placeholder image'\]/)
  assert.match(source, /class="absolute inset-0 z-10 grid place-items-center p-6"/)
  assert.match(source, /<MessageNotification[\s\S]*:messages="placeholderMessages"[\s\S]*:autoplay="false"[\s\S]*:loop="false"[\s\S]*:max-visible="1"/)
