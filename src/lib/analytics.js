...
export function sanitizePostHogEvent(payload, configuredKey) {
  ...
  copyDefined(properties, normalizeAnalyticsProperties(source), productProperties)
...
export function capturePageview(firstTouch = getFirstTouch()) {
  ...
  const landingPath = properties.landing_path
  if (!landingPath || landingPath === lastPageviewPath) return
  lastPageviewPath = landingPath
...
export function trackOnboardingCta(event, entry, cta) {
  if (!entry?.available) {
