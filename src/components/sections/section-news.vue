...
<template>
  <section id="news" :class="cn('w-full scroll-mt-24 px-6 py-24')" data-section-news>
    <div class="w-full mx-auto max-w-(--content-width)">
      ...
      <div class="mt-12 grid grid-cols-1 gap-8 md:grid-cols-2">
        <a v-for="post in newsPosts" :key="post.path" :href="post.path"
          class="block min-w-0 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">
          <div class="aspect-3/2 overflow-hidden rounded-2xl bg-muted">
            <img :src="getPostImage(post)" :alt="getPostImageAlt(post)" class="size-full object-cover" loading="lazy"
              decoding="async">
          </div>
          ...
          <div class="mt-5">
            ...
            <p
              class="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm font-medium leading-none text-muted-foreground">
