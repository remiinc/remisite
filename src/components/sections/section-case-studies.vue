...
<template>
  <section
    ...
    id="case-studies"
    :class="cn('w-full scroll-mt-24 overflow-hidden px-6 py-24')"
      ...
      >
        <div class="w-full flex gap-0">
          <article
            ...
            :key="caseStudy.path"
            role="group"
            :aria-label="`${index + 1} of ${visibleCaseStudies.length}`"
            class="min-w-0 w-1/3 pr-2 shrink-0"
            :aria-current="selectedIndex === index ? 'true' : undefined"
          >
            <a
              :href="caseStudy.path"
              :aria-label="`Read case study: ${caseStudy.title}`"
              class="group w-full relative block aspect-5/7 overflow-hidden rounded-3xl bg-muted text-background focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
            >
              <img
                v-if="getImageUrl(caseStudy)"
              ...
              <h3 class="absolute inset-x-0 bottom-0 p-5 text-2xl font-normal leading-none tracking-tight text-balance">
                {{ caseStudy.title }}
              </h3>
            </a>
          </article>
