    ...
    <div :class="cn(
      ...
      <nav :class="cn(
        ...
        <Teleport to="body" :disabled="!isMobileMenuVisible">
          <div id="site-navigation" :class="cn(
            isMobileMenuVisible
          ...
          )" :role="isMobileMenuVisible ? 'dialog' : undefined" :aria-modal="isMobileMenuVisible ? 'true' : undefined"
            :aria-label="isMobileMenuVisible ? 'Mobile navigation' : undefined">
            <div
              class="flex shrink-0 items-center justify-between px-6 pb-4 pt-[max(1rem,env(safe-area-inset-top))] md:hidden">
              <a v-if="!activeMobilePanel" href="/"
                ...
                aria-label="Remi home" @click="closeMobileMenu">
            ...
            <nav
              ...
              <div
                class="h-full divide-y divide-foreground/5 overflow-y-auto px-6 md:flex md:h-auto md:items-center md:gap-0 md:divide-y-0 md:overflow-visible md:border-y-0 md:p-0">
                <template v-for="link in visibleMenuLinks" :key="link.label">
                  <a v-if="!link.columns" :href="link.href"
                  ...
                  <div v-else class="group md:relative">
                    <button type="button"
                      ...
                      :aria-expanded="activeMobilePanel === link.panel" @click="openMobilePanel(link.panel)">
                      <span class="relative z-1">
                        {{ link.label }}
                      </span>
                      <PhCaretRight aria-hidden="true" class="size-5 md:hidden" />
                      <PhCaretDown aria-hidden="true" weight="bold"
                    ...
                    <div :class="cn(
                      ...
                      <div
                        class="md:rounded-xl md:bg-background md:p-6 md:text-foreground md:shadow-[0_2em_5em_rgba(0,0,0,0.1)]">
                        <div class="flex flex-col gap-10 md:grid md:grid-cols-[0.9fr_1.2fr] md:gap-4">
                          <div v-for="column in link.columns" :key="column.label"
                            class="flex flex-col md:[&:has(a:hover)_a:not(:hover)]:opacity-50">
                            <span class="mb-5 text-xs text-muted-foreground/50 md:mb-4">
                              {{ column.label }}
                            </span>

                            <a v-for="item in column.links" :key="item.label" :href="item.href" :class="cn('group/link border-b border-border py-3 transition-[opacity,color] hover:text-foreground hover:opacity-100 focus-visible:outline-none md:border-b-0 md:hover:text-foreground md:hover:opacity-100 md:focus-visible:text-foreground md:focus-visible:opacity-100',
                              column.variant === 'featured' && 'md:pb-[0.25em] md:pt-0',
                              column.variant === 'compact' && 'md:py-[0.2em] md:text-sm'
                            )" @click="closeMobileMenu">
                              <span :class="cn(
                                'text-xl font-medium leading-tight tracking-[-0.04em] text-foreground md:tracking-normal',
                                'flex items-center gap-0',
                                column.variant === 'featured' && 'md:text-xl md:font-medium md:text-foreground',
                                column.variant === 'compact' && 'md:text-sm md:font-normal md:leading-tight md:text-muted-foreground group-hover/link:text-foreground group-focus-visible/link:text-foreground'
                              )">
                                <span
                                  class="grid min-w-0 grid-cols-[0fr] group-hover/link:grid-cols-[1fr] group-focus-visible/link:grid-cols-[1fr] transition-[grid-template-columns] duration-200 ease-in-out">
                                  <span class="overflow-hidden">
                                    <span class="block pr-2">
                                      <PhArrowRight aria-hidden="true" class="size-lh" weight="regular" />
                                    </span>
                                  </span>
                                </span>
                                <span>
            ...
            <div
              class="grid shrink-0 grid-cols-2 gap-3 px-6 pt-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] md:hidden">
              <Button :href="portalLoginHref" target="_blank" variant="primary" class="h-12 w-full"
                @click="closeMobileMenu">
                Login
              </Button>
              <Button href="/qualify" variant="secondary" class="h-12 w-full" @click="closeMobileMenu">
                Try for free
              </Button>
            </div>
          </div>
        ...
        <div class="flex flex-1 items-center justify-end md:hidden">
          <button type="button"
            ...
            :aria-expanded="isMobileMenuOpen" aria-controls="site-navigation" aria-label="Open menu"
